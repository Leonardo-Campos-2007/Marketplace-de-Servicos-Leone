...
- Armazenamento abstraído atrás da interface `ArmazenamentoService`, com implementação inicial `ArmazenamentoLocalService` gravando em disco. **Isso não é solução de produção** — em ambiente containerizado (Docker/Kubernetes), o filesystem é efêmero e um restart apaga os arquivos. Trocar para S3/Cloudinary/equivalente antes de qualquer deploy real; a interface já isola essa troca do resto do sistema.
- Whitelist restrita a `image/jpeg`, `image/png`, `image/webp`. Limite de negócio de 5MB por arquivo.
- **Validação por magic bytes (assinatura real dos primeiros bytes do arquivo), nunca por extensão do nome nem por `Content-Type` declarado pelo cliente.** Verificação manual (JPEG `FF D8 FF`, PNG `89 50 4E 47`, WebP via `RIFF`/`WEBP`), sem dependência nova.
- Nome de arquivo do cliente nunca vira path real — sempre substituído por UUID interno antes de tocar o filesystem, com `ArmazenamentoLocalService.salvar` reforçando defesa contra Path Traversal via `normalize()` + `startsWith(diretorioBase)`, mesmo confiando que a entrada já deveria ser um UUID seguro.
- Download servido por endpoint dedicado autenticado (`GET .../anexo/download`), não por path estático — permite aplicar a mesma checagem de `validarParticipante` usada no resto do chat. **Dívida técnica conhecida:** o endpoint de download usa `FileSystemResource` diretamente a partir de `anexo.getUrlArquivo()`; se `ArmazenamentoService` for trocado para S3 no futuro, esse trecho do Controller precisa mudar também, porque só abstraímos "salvar" e "deletar", não "servir".
...
- **Upload de arquivo (multipart) passa por múltiplas camadas de validação antes de chegar no código de negócio, e cada uma sem handler mapeado produz erro genérico (visto como 403 vazio) em vez do status correto.** Ao implementar `Anexo`, dois erros do próprio Spring escaparam sem tratamento: `MaxUploadSizeExceededException` (limite de tamanho do servidor, `spring.servlet.multipart.max-file-size`, que por padrão é bem menor que o limite de negócio de 5MB) e `HttpMediaTypeNotSupportedException` (o Spring rejeita o `Content-Type` **declarado** pela parte do multipart antes mesmo de qualquer byte do arquivo ser lido pelo código customizado). **Regra prática: toda vez que uma funcionalidade nova usa um mecanismo de framework com validação própria (multipart, upload, parsing de formato), mapear explicitamente as exceções desse mecanismo no `GlobalExceptionHandler`, não assumir que só as exceções de domínio escritas à mão vão ocorrer.**
- **`Content-Type` declarado no multipart e magic bytes reais são validados em momentos diferentes — testar "arquivo forjado" exige que os dois divirjam, não só um.** Um teste de segurança que pretende provar "magic bytes vencem sobre extensão" precisa que o Content-Type declarado pareça válido (ex.: `image/jpeg`, o que depende da **extensão real do nome do arquivo**, não de renomear só a exibição ou de embutir "jpg" no meio do nome como `teste.jpg.txt`) para passar da checagem inicial do Spring e chegar na validação de bytes do código customizado. Se o Content-Type declarado já for rejeitado antes (ex.: `.txt` puro gerando `text/plain`), o teste está validando a camada errada — ainda é uma proteção real, mas não a que a fase pretendia provar.
